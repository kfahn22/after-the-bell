...
float checkSymmetry( vec2 uv) {
    ...
    float l = l1 + l2  + l3 + l4 + l5 + l6 + l7 + l8 + l9 
    ...
    float v = v16 + v17 + v18 + v19 + v20 + v21 + v22
...
float sdSquareLoop( vec2 uv, float a, float b, float c, float d ) {
  // left vertical
   float s1 = sdBox( uv - vec2(a-.25, b+.0), vec2(c, 2.75*d) );
   float m1 = S(0.008, 0.0, s1);
   // right vertical
   float s2= sdBox( uv - vec2(a , b - .2), vec2(c, 4.75*d) );
   float m2 = S(0.008, 0.0, s2);
   // top horizontal
   float s3= sdBox( uv - vec2(a - .125, b + .25), vec2(1.*d, c) );
...
float sdSquareLoops( vec2 uv, float x, float y) {
    float m1 = sdOutline(uv, -x, y, vec2(.10, .10) );
    float m2 = sdOutline(uv, x, y, vec2(.10, .10) );
    float s3 = sdRoundedBox(uv - vec2(-.1, .0), vec2(.02, .15), vec4(.01, .02, .01, .01));
    float m3 = S(.008, 0.0, s3);
    return m1 + m2 + m3;
}
...
//   col += chooseShape( shapechoice, uv, bkcol, shapecol );
    vec2 st = vec2(abs(uv.x), uv.y);
    float m = sdSquareLoop( uv, 0.0, 0.0, .025, .1);
    col += (1. - m) * GREY + m * TEAL;
